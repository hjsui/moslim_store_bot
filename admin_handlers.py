...
def register_admin_handlers(bot):
    ...
    def admin_ff_add_prompt(call):
        if call.from_user.id != OWNER_ID:
            bot.answer_callback_query(call.id, "غير مسموح", show_alert=True)
            return
        admin_temp[call.from_user.id] = {'action': 'ff_add', 'step': 'awaiting_code'}
    ...
    def admin_ff_del_prompt(call):
        if call.from_user.id != OWNER_ID:
            bot.answer_callback_query(call.id, "غير مسموح", show_alert=True)
            return
        admin_temp[call.from_user.id] = {'action': 'ff_del', 'step': 'awaiting_code'}
    ...
    def admin_keys_add_prompt(call):
        if call.from_user.id != OWNER_ID:
            bot.answer_callback_query(call.id, "غير مسموح", show_alert=True)
            return
        admin_temp[call.from_user.id] = {'action': 'keys_add', 'step': 'awaiting_code'}
    ...
    def admin_keys_del_prompt(call):
        if call.from_user.id != OWNER_ID:
            bot.answer_callback_query(call.id, "غير مسموح", show_alert=True)
            return
        admin_temp[call.from_user.id] = {'action': 'keys_del', 'step': 'awaiting_code'}
...
def process_admin_text(bot, message):
    ...
    if len(parts) != 2:
        bot.reply_to(message, "❌ الصيغة غير صحيحة. أرسل: `الكمية الكود` أو `المدة الكود`", parse_mode="Markdown")
        return
    ...
    if action == 'ff_add':
        ...
        if del_ff_code(first, second):
            ...
            bot.reply_to(message, f"❌ الكود غير موجود")
        del admin_temp[user_id]
