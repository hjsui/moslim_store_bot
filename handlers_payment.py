# handlers_payment.py
# دوال نظام الدفع (طرق الدفع، قبول/رفض الطلبات، إثباتات) - تم تعديله لجعل الدوال عامة
...
# ========== دوال عامة يمكن استيرادها من ملفات أخرى ==========

def show_payment_methods(user_id, product_type, product_id, amount, bot):
...
def purchase_ff_package(user_id, pkg, lang, bot):
    from config import prices
    amount = prices[pkg]
    show_payment_methods(user_id, 'ff', pkg, amount, bot)

def purchase_key(user_id, days, lang, bot):
    from config import keys_inventory
    price = keys_inventory['dripclient']['prices'][days]
    show_payment_methods(user_id, 'key', f"dripclient_{days}", price, bot)

def send_withdrawal_log(admin_username, product_name, price, extra_info="", code_or_link=None, bot=None):
...
# ========== دوال خاصة بالمعالجات الداخلية (لا يتم استيرادها) ==========

def finalize_order(order_id, accepted, admin_id=None, bot=None):
...
# ========== دالة تسجيل معالجات الدفع ==========

def register_payment_handlers(bot):
    """تسجيل معالجات نظام الدفع (الكول باكات فقط)"""
    ...
    def change_payment_method(call):
        ...
        show_payment_methods(user_id, product_type, product_id, amount, bot)
    ...
    def admin_accept_order(call):
        if not is_admin(call.from_user.id):
            ...
            return
        order_id = call.data.split('_', 2)[2]
        finalize_order(order_id, accepted=True, admin_id=call.from_user.id, bot=bot)
    ...
    def admin_reject_order(call):
        if not is_admin(call.from_user.id):
            ...
            return
        order_id = call.data.split('_', 2)[2]
        finalize_order(order_id, accepted=False, admin_id=call.from_user.id, bot=bot)
        bot.answer_callback_query(call.id, "❌ تم رفض الطلب")

    # نعيد قاموساً يحتوي على الدوال التي قد يحتاجها ملفات أخرى (اختياري)
    return {
